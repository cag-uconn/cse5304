...
__global__ void tma_multiwarp_pipeline(
...
__global__ void
simple_vector_copy(bf16 *__restrict__ dest, const bf16 *__restrict__ src, int N) {
...
#define BENCHMARK_KERNEL(kernel_call, num_iters, size_bytes, label) \
    do { \
        ...
        printf( \
            "%s - Time: %.4f ms, Bandwidth: %.2f GB/s\n", \
            label, \
            time_per_iter, \
            bandwidth_gb_s); \
...
int main() {
    ...
    CUDA_CHECK(cudaMemcpy(d_src, matrix, size * sizeof(bf16), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(
        cudaMemcpy(tma_result, d_dest, size * sizeof(bf16), cudaMemcpyDeviceToHost));

    bool tma_correct = true;
    for (int idx = 0; idx < size; idx++) {
        if (tma_result[idx] != matrix[idx]) {
            printf(
                "First mismatch at [%d]: %.4f != %.4f\n",
                idx,
    ...
    CUDA_CHECK(cudaMemset(d_dest, 0, size * sizeof(bf16)));
    simple_vector_copy<<<size / (elem_per_block * 32), 32>>>(d_dest, d_src, size);
    CUDA_CHECK(cudaDeviceSynchronize());

    bf16 *simple_result = (bf16 *)malloc(size * sizeof(bf16));
    CUDA_CHECK(
        cudaMemcpy(simple_result, d_dest, size * sizeof(bf16), cudaMemcpyDeviceToHost));
