...
struct MatmulTensor {
    ...
    run(int32_t size_i,
        ...
        float *c,
        void *workspace) {
        matmul_tensor::launch_matmul_tensor(size_i, size_j, size_k, a, b, c);
    }
};

BenchmarkResults get_cublas_fma_results() {
    // Hard-coded data collected on A6000 GPU (Ding Server)
